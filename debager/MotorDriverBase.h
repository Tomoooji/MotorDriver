...
class MotorDriverBase{
  protected:
    enum Mode{INVALID,DIGITAL,ANALOG} _MODE;
    uint8_t _pin1, _pin2;
    const uint8_t _MIN;
    const uint8_t _MAX;
    int _velocity;
    ...
    float _decel;
  public:
    MotorDriverBase(const int mode = 1, const uint8_t MIN = LIMIT_MIN, const uint8_t MAX = LIMIT_MAX);
    virtual bool attach(uint8_t pin1, uint8_t pin2); // returns ware pins correct
     Mode checkPin(uint8_t pin){return isInputOnly(pin)? INVALID: (isDigitalOnly(pin)? DIGITAL: ANALOG);}
